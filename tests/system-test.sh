...
echo "Step 5: Cloning the second repository using gitception:"
{
    ...
    EOF
        ...
        local total_files=$(( num_commits * files_per_commit ))

        local random_data_size=$(( total_files * random_data_per_file ))

        local random_data_file="${tempdir}/data"

        head -c "${random_data_size}" "${random_source}" > "${random_data_file}"

    

        echo "Step 1 (multikey): Creating new GPG keys to use for testing:"
        ...
        echo "Step 2 (multikey): Creating new repository with random data:"
        ...
        echo "Step 3 (multikey): Creating an empty bare repository to receive pushed data:"
        ...
        echo "Step 4 (multikey): Pushing the first repository to the second one using gitception:"

        {
            ...
            (
                ...
                git config gcrypt.participants "${key_fps[9]}"
        ...
        echo "Step 5 (multikey): Test that clone fails with no matching key:"

        {

            local original_gnupghome="${GNUPGHOME}"
            ...
            echo "Attempting to clone with an empty GPG keyring. This should fail."
            ...
            (
                ...
                if [ $exit_code -eq 0 ]; then
                    ...
                    echo "Clone succeeded unexpectedly!"

                    exit 1

                else

                    echo "Clone failed as expected."
        ...
        echo "Step 6 (multikey): Cloning the second repository using gitception:"
